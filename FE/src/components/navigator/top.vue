        ...
        <b-navbar toggleable="md" type="dark" variant="secondary">
            ...
            <b-collapse is-nav id="nav_collapse">
                <b-navbar-nav class="ml-auto">
                    <button type="button" class="btn navbtn btn-primary" v-if="loginStatus==false"
                            v-on:click=loginWithKakao(1)>로그인
                    </button>
                    <button type="button" class="btn navbtn btn-primary" v-if="loginStatus==false"
                            v-on:click=loginWithKakao(0)>회원가입
                    </button>
                    ...
                    <b-nav-item-dropdown right v-if="loginStatus">
                        <template slot="button-content">
                            <em>리뷰하기
                                <b-badge variant="light" v-if="groupReviewList.length!=0" >NEW</b-badge>
                            </em>
                        </template>
                    ...
                    <b-nav-item-dropdown right v-if="loginStatus">
                        <template slot="button-content">
                            <b-img class="profileImg" rounded="circle" width="40px" height="40px"
                                   stype="margin-right:5px" :src="userInfo[1]"/>
                            <em> {{userInfo[0]}}</em>
                        </template>
    ...
    export default {
        ...
        methods: {
            ...
            logoutWithKakao() {
                ...
                this.$socket.emit('logout', this.$session.get('userID'))
                Kakao.Auth.cleanup();
