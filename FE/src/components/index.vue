<template>
    <div id="homePage">
        <modals-container/>

        <div class="mainTitleArea">
            <div class="mainTitle">
                Connect PnP
                <img src="../assets/connectPnP.png"/>
            </div>
        </div>

        <div class="searchArea">
            <form action="#">
                <b-input-group>
                    <b-form-input type="text" v-model="searchQuery" placeholder="Search"/>
                    <b-input-group-append>
                        <b-btn variant="info" @click="searchGroup(true)">검색</b-btn>
                    </b-input-group-append>
                </b-input-group>
            </form>
        </div>

        <!--  카테고리 리스트가 보여지는 부분   -->
        <b-container class="categoryGroup">
            <b-row>
                <div v-for="item in categoryList" :key="item._id" @click="goCategoryList(item._id)">
                    <b-col sm class="homePageCategory categorySingle">
                        <input type="image"
                               :src="item.img_path"
                               height="120px"
                               width="120px"/>
                        <p>{{item.name}}</p>
                    </b-col>
                </div>
            </b-row>
        </b-container>

        <div>
            <button v-on:click='moreBtn' type="button" class="btn btn-outline-info moreButton">더보기</button>
        </div>
        <hr>

        <div class="rankingArea">

            <div>
                <h1><b>Group Ranking Best 5</b></h1>
                <h4><b>From Whole Group</b></h4>
            </div>
            <!--justify-content-md-center-->
            <b-container fluid style="margin-bottom: 100px">
                <b-row class="rankingGroup justify-content-md-center">
                    <div class="singleRanking" v-for="item in rankingList" :key="item.id">
                        <b-col >
                            <b-img rounded fluid :src="item.images[0]" style="max-height: 200px;" alt="Thumbnail"/>
                            <div class="singleRankingTitile">{{item.title}}</div>
                            <div class="singleRankingDescription">{{item.location.title}}</div>
                            <router-link :to="{name: 'detailParty' , params: {id: item._id}}" class="btn-info"
                                         tag="b-button">자세히 보기
                            </router-link>
                        </b-col>
                    </div>
                </b-row>
            </b-container>


            <hr>

            <div>
                <h1 style="margin-top: 100px"><b>Recommended Group Best 5</b></h1>
                <h4><b>Only For You</b></h4>
            </div>
            <!--justify-content-md-center-->
            <b-container fluid>
                <b-row class="rankingGroup justify-content-md-center">
                    <div v-if="recommendedList.length==0" style="font-size: larger;">Please Choose Interesting
                        Category
                    </div>
                    <div  class="singleRanking" v-for="item in recommendedList" :key="item.id">
                        <b-col>
                            <b-img rounded fluid :src="item.images[0]" style="max-height: 200px;" alt="Thumbnail"/>
                            <div class="singleRankingTitile">{{item.title}}</div>
                            <div class="singleRankingDescription">{{item.location.title}}</div>
                            <router-link :to="{name: 'detailParty' , params: {id: item._id}}" class="btn-info"
                                         tag="b-button">자세히 보기
                            </router-link>
                        </b-col>
                    </div>
                </b-row>
            </b-container>

        </div>

    </div>

</template>

<script>
    import Review from './reviewPopup/Review.vue'

    const config = require('../server.config');
    var tempimg = "data:image/svg+xml;utf8;base64,"
    export default {
        name: 'homePage',
        components: {
            Review
        },
        data() {
            return {
                searchQuery: '',
                categoryPage: 0,
                btnclicked: 5,
                categoryList: [],
                rankingList: [],
                recommendedList: [],
                id: String,
                usrName: String,
                img: String,
                userInterestedCategory: [],
            }
        },
        beforeMount() {
            this.id = this.$session.get('userID');
        },
        mounted: function () {
            this.getCategoryList();
            this.getGroupRanking();
            //this.getRecommendGroup();
        },
        methods: {
            //"더보기" 버튼이 클릭되면 보여지는 카테고리 수가 5개씩 늘어나도록
            moreBtn() {
                this.categoryPage++
                this.getCategoryList()
            },
            goCategoryList(categoryId) {
                this.$session.set('category', categoryId);
                window.location.href = config.serverFE() + "party/list"
            },
            searchGroup() {
                this.$session.set('mainQuery', this.searchQuery);
                this.$session.set('category', null);
                window.location.href = config.serverFE() + "party/list"
            },
            getGroupRanking() {
                // 모든 그룹 중에 top 5
                this.$http.get(config.serverUrl() + `home`)
                    .then((result) => {
                        this.rankingList = result.data.slice(0, 5);

                        if (this.id != null) {
                            // 나의 관심카테고리 중에 top 5
                            this.$http.get(config.serverUrl() + 'user/details/' + this.id)
                                .then((userInfo) => {

                                    // 관심 카테고리 가져오기
                                    var catListtemp = [];
                                    for (var j = 0; j < userInfo.data.categoryList.length; j++) {
                                        catListtemp.push(userInfo.data.categoryList[j]);
                                    }
                                    this.userInterestedCategory = catListtemp;

                                    var num = 0;
                                    var id = this.$session.get('id');

                                    for (var i = 0; i < result.data.length; i++) {
                                        if (num == 5) break;
                                        if ((catListtemp.indexOf(result.data[i].category) != -1) && (result.data[i].host != id)) {
                                            this.recommendedList.push(result.data[i]);
                                            num++;
                                        }
                                    }
                                });
                        }
                    })
            },
            getCategoryList() {
                var vm = this
                this.$http.get(config.serverUrl() + 'category/list/' + this.categoryPage)
                    .then((result) => {
                        // get category list
                        for (var i = 0; i < result.data.length; i++) {
                            vm.categoryList.push(result.data[i])
                        }
                    })
            }
        }
    }
</script>

<style scoped>
    hr {
        border-top: 5px dashed #17a2b8;
    }

    span {
        position: absolute;
        width: 100%;
        height: 100%;
        top: 0;
        left: 0;

        z-index: 1;
    }

    .mainTitleArea {
        width: 100%;
        height: 200px;
        position: relative;

    }

    .mainTitle {
        color: #000000;
        font-size: 80px;
        font-weight: bold;
        padding-bottom: 30px;
        margin-top: 90px;
    }

    .searchArea {
        width: 50%;
        margin-left: 25%;
        padding-bottom: 50px;
    }

    .categoryGroup {

        width: 1000px;
        position: relative;
    }

    .categorySingle {
        border-radius: 40%;
        padding-top: 5px;
        height: 160px;
        width: 160px;
        background-color: #17a2b8;
        margin: 20px 20px;
        text-align: center;
        color: white;
    }

    .moreButton {
        margin: 50px 20px;
    }

    .rankingArea {
        width: 100%;
        margin-bottom: 100px;
        padding-top: 60px;
        padding-bottom: 100pt;
    }

    .rankingGroup {
        width: 100%;
        margin-top: 30px;
        margin-bottom: 30px;

    }

    .singleRanking {
        padding: 10px;
        background-color: white;
        border: 2px solid #d2e8ff;
        margin-right: 20px;
    }

    .singleRankingTitile {
        margin-top: 5px;
        margin-bottom: 5px;
        font-size: 30px;
        font-weight: bold;
    }

    .singleRankingDescription {
        max-width: 250px;
        max-height: 30px;
        overflow: hidden;
        font-size: 20px;
        margin-bottom: 20px;
    }

    .rankingSingle {

        margin: 20px 20px;
    }

</style>
